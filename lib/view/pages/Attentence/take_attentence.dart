...
import 'package:excelkarror_application/controllers/userCredentials/user_credentials.dart';
import 'package:excelkarror_application/model/attendence_model/attendence-model.dart';
import 'package:excelkarror_application/view/colors/colors.dart';
import 'package:excelkarror_application/view/constant/sizes/sizes.dart';
...
class _TakeAttenenceScreenState extends State<TakeAttenenceScreen> {
    ...
    try {
      final reponse = await http.post(
        ...
        headers: <String, String>{
          ...
          'Authorization':
              'AAAATw1EhEw:APA91bHD0VxNJq-Mtjm0jNVcpr5226AUK3CqfTDzr792Kr0JmdFfjX8CZR3bEu93ESVnAU3Qz6Ysklp0DZBvXbu-x-lVk-uuk7yE9JxL_LkRL2bfP2nDpANqPnpnvEoUcyBzLEOBkZCf'
        },
        body: jsonEncode(
          <String, dynamic>{
            'priority': 'high',
  ...
  @override
  void initState() {
    allStudentPresentFunction().then((value) {
      setState(() {});
    });
    super.initState();
    getSchoolTimer();
  ...
  Future allStudentPresentFunction() async {
    var snapshoting = await FirebaseFirestore.instance
        .collection("SchoolListCollection")
        .doc(widget.schoolID)
        .collection(widget.batchId)
        .doc(widget.batchId)
    ...
    for (var i = 0; i < snapshoting.docs.length; i++) {
      presentlist[snapshoting.docs[i]['studentName']!] = true;

      final indexx = attendanceList
          .indexWhere((element) => element.uid == snapshoting.docs[i]['docid']);
      ...
      if (indexx != -1) {
        ...
        attendanceList.add(AttendanceStudentModel(
            Date: "date.toString()",
            present: true,
            studentName: snapshoting.docs[i]['studentName'],
            uid: snapshoting.docs[i]['docid']));
      }
    }
    // log("message$presentlist");
  }
}
